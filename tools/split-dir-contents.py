...
def main():
    parser = argparse.ArgumentParser(description='tool to split directory into multiple directories by copying files',
                                     epilog='USE WITH EXTRA CARE, always test with --dry-run')
    parser.add_argument('-i', '--input-dir', help='input directory from which you want to copy files', default='.')
    parser.add_argument('-o', '--output-prefix', help='prefix name of output directories', required=False, default='')
    parser.add_argument('-n', '--chunk-size', help='copy up to NUMFILES of files to each new directory',
                        metavar='NUMFILES', type=int, required=True)
    parser.add_argument('-f', '--force', action='count', default=0, help='allow copying to (-f) and overwriting files '
                                                                         '(-ff) in non-empty output directories')
    parser.add_argument('-m', '--move', help='move files instead of copying', action='store_true')
    parser.add_argument('-D', '--dry-run', help='dry run to show you what would happen', action='store_true')
    parser.add_argument('-A', '--all', help='also copy hidden files', action='store_true')
    ...
    if args.dry_run:
        ...
        if args.move:
            def process_file(filepath, destination):
                ...
                if os.path.exists(os.path.join(destination, fname)):
                    if args.force > 1:
                        ...
                        warning("Not moving file %s because same name is used in destination directory %s (-ff to "
            ...
            def process_file(filepath, destination):
                fname = os.path.basename(filepath)
                if not os.path.exists(os.path.join(destination, fname)) or args.force > 1:
        ...
        if args.move:
            ...
            def process_file(filepath, destination):
                ...
                if not os.path.exists(os.path.join(destination, fname)) or args.force > 1:
                    ...
                    warning("Not copying file %s because same name is used in destination directory %s (-ff to "
                            "overwrite)" % (filepath, destination))
        ...
        if not os.path.exists(destination):
            if args.dry_run:
                ...
                try:
                    ...
                    print("Wasn't able to create directory ", destination, ":", sep='', file=sys.stderr)
                    sys.exit(e)

        if os.path.isdir(destination):
            if os.listdir(destination):
                if args.force > 0:
                    warning("Directory already exists and contains some files: %s. CONTINUING due to --force" % destination)
                else:
                    sys.exit("Directory already exists and contains some files: %s.\n"
                             "You can:\n\t1) use (some other) --output-prefix\n"
                             "\t2) manually clean up your files\n"
                             "\t3) specify -f or --force to allow copying to non-empty directories" % destination)
        else:
            if not args.dry_run:
    ...
    message = "%d files from %s to %s" % (len(filenames), args.input_dir, args.output_prefix + '0')
