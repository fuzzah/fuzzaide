        ...
        if args.names:
            ...
            if len(names) != num_parts:
                sys.exit(
                    ...
                    "argument, but now you have %d file parts and %d names!"
                    % (num_parts, len(names))
                )
            ...
            if not os.path.isdir(args.output_prefix):
                ...
                if not args.dry_run:
                    try:
                        ...
                        print(
                            "Wasn't able to create output directory '%s'. Error follows:"
                            % args.output_prefix,
                            file=sys.stderr,
                        )
                        sys.exit(1)
